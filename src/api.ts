...
export function getMovies(type:string){
  return fetch(`${BASE_URL}/movie/${type}?api_key=${API_KEY}&language=ko&page=1&region=kr`).then(resp => resp.json());
}

export function getSearch(keyword:string){
  return fetch(`${BASE_URL}/search/multi?api_key=${API_KEY}&language=ko&query=${keyword}&include_adult=true&region=kr`).then(resp=>resp.json());
}
...
export function getTvSeries(type:string){
  return fetch(`${BASE_URL}/tv/${type}?api_key=${API_KEY}&language=ko`).then(resp=>resp.json());
}
