...
export const BigCover = styled.div<{ bgPhoto: string }>`
  background: linear-gradient(
    ...
    url(${(props) => props.bgPhoto}) center center/cover no-repeat;
  padding-top: 56.25%;
  width: 100%;
  position: relative;
`;
...
function BigMovie({ clickedId }: IBigMovieProps) {
  ...
  const { data: detail, isLoading: bigIsLoading } = useQuery<IGetMovieDetail>(
    ["movie", "detail"],
    () => getMovieDetail(clickedId),
    { enabled: !!clickedId }
  );
  const { data: credits, isLoading: creditsLoading } = useQuery<IMovieCredits>(
    ["movie", "credit"],
    () => getMovieCredits("movie", clickedId),
    { enabled: !!clickedId }
  );
              ...
              >
                <BigTitle>
                  ...
                  {detail.original_title ? (
                    <h4>
                      {detail?.original_title} ・{" "}
                      {detail?.release_date.toString().substring(0, 4)}
                    </h4>
                  ) : null}
