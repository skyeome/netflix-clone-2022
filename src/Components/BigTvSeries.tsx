...
import { getMovieCredits, getTvDetail, IGetTvDetail, IMovieCredits } from "../api";
import { breakpoints, SearchItem } from "../Routes/Search";
import { makeImagePath } from "../utils";
import { ActorName, BigContent, BigCover, BigOverView, BigTitle, CharacterName, Genres, HomeLink, MovieModal, Overlay, ScrollArea, SmallTitle } from "./BigMovie";
...
function BigTvSeries({clickedId}:IBigTvProps){
  ...
  const {data:credits,isLoading:creditsIsLoading} = useQuery<IMovieCredits>(["tv","credits"],()=>getMovieCredits("tv",clickedId));
      ...
      <ScrollArea>
        {detail ? <>
          ...
          <BigContent>
            ...
            <Episode>
              <StillImg>
                {detail.last_episode_to_air.still_path ? <img src={makeImagePath(detail.last_episode_to_air.still_path || "","w500")} /> : <img src={process.env.PUBLIC_URL + "/video-icon-image.jpg"} alt="no thumbnail" />}
              </StillImg>
              <EpisodeTxt>
                <h5>{detail.last_episode_to_air.episode_number}화 {detail.last_episode_to_air.name ? " ・ " + detail.last_episode_to_air.name : null}</h5>
                <p>방영일 : {new Date(detail.last_episode_to_air.air_date).toLocaleDateString()}</p>
